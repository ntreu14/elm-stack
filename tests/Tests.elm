    ...
    , describe "toList"
        [ fuzz (list int) "the size should be the size of the original list" <|
            \xs ->
                Expect.equal
                  (List.length xs)
                  (Stack.fromList xs |> Stack.toList |> List.length)
      ...
      , describe "append" 
        ...
        , describe "concat"
            [ fuzz (list (list int)) "concating stacks is the same as appending lists together" <|
                \xs ->
                  let
                    flattenedList = List.foldr List.append [] xs 
                  in
                    Expect.all
                      [ Expect.equal (Stack.fromList flattenedList)
                      , Expect.equal (List.length flattenedList) << Stack.size
                      ]
